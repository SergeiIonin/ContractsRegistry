...
class ContractServiceTestImpl[F[_] : Monad] extends ContractService[F]:
  ...
  override def saveContract(contract: Contract): EitherT[F, HttpErrorDTO, Unit] = 
    EitherT(
      storage.add(contract.subject, CreateContractDTO(subject = contract.subject, schema = contract.schema))
  ...
  override def getSubjects(): EitherT[F, HttpErrorDTO, List[String]] =
    ...
    if subjects.isEmpty then EitherT(asLeft[List[String]](BadRequestDTO(404, "No subjects found")).pure[F])
  ...
  override def getLatestContract(subject: String): EitherT[F, HttpErrorDTO, ContractDTO] = 
    storage.getVersions(subject) match
      case Right(versions) => 
        if versions.isEmpty then EitherT(asLeft[ContractDTO](BadRequestDTO(404, s"No versions found for subject $subject")).pure[F])
