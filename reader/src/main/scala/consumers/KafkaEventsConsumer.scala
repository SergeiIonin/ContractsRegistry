...
abstract class KafkaEventsConsumer[F[_], K, V](kafkaConsumer: KafkaConsumer[F, K, V])
    extends Consumer[F]:
...
object KafkaEventsConsumer:
  ...
  given deserializerContractCreateRequestedKey[F[_]: Async]
      : Deserializer[F, ContractCreateRequestedKey] =
    Deserializer.lift(key => fromEither(key))

  given deserializerContractCreateRequested[F[_]: Async]
      : Deserializer[F, ContractCreateRequested] =
    Deserializer.lift(event => fromEither(event))

  given deserializerContractDeleteRequestedKey[F[_]: Async]
      : Deserializer[F, ContractDeleteRequestedKey] =
    Deserializer.lift(key => fromEither(key))

  given deserializerContractDeleteRequested[F[_]: Async]
      : Deserializer[F, ContractDeleteRequested] =
    Deserializer.lift(event => fromEither(event))
