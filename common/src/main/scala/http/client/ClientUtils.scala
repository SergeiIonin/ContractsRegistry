...
object ClientUtils:
  ...
  private val gitHubApiVersionHeader =
  ...
  def postRequest[F[_], T](uri: Uri, token: Option[String], entity: T)(
      using EntityEncoder[F, T]): Request[F] =
    Request[F](Method.POST, uri, headers = getHeaders(token)).withEntity(entity)
