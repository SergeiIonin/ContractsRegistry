...
final class ContractCreateKafkaEventsProducer[F[_]: Async](
    ...
    override val kafkaProducer: KafkaProducer[
      F,
...
object ContractCreateKafkaEventsProducer:
  ...
  )(using Serializer[F, ContractCreateRequestedKey], Serializer[F, ContractCreateRequested])
      : Resource[F, EventsProducer[F, ContractCreateRequestedKey, ContractCreateRequested]] =
    KafkaProducer[F]
      .resource[ContractCreateRequestedKey, ContractCreateRequested](
        producerSettings(bootstrapServers))
      .map(kafkaProducer =>
        ContractCreateKafkaEventsProducer[F](contractCreatedTopic, kafkaProducer))
