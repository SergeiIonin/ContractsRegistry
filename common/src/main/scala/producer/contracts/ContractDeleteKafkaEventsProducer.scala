...
object ContractDeleteKafkaEventsProducer:
  ...
  )(using Serializer[F, ContractDeletedEventKey], Serializer[F, ContractDeletedEvent])
      : Resource[F, EventsProducer[F, ContractDeletedEventKey, ContractDeletedEvent]] =
    KafkaProducer[F]
      .resource[ContractDeletedEventKey, ContractDeletedEvent](
        producerSettings(bootstrapServers))
      .map(kafkaProducer =>
        ContractDeleteKafkaEventsProducer[F](contractCreatedTopic, kafkaProducer))
