...
object KafkaEventsProducer:
  def producerSettings[F[_], K, V](bootstrapServers: String)(
      using Serializer[F, K],
      Serializer[F, V]): ProducerSettings[F, K, V] =
  ...
  given serializerContractCreateRequestedKey[F[_]: Async]
      : Serializer[F, ContractCreateRequestedKey] =
    Serializer.lift(key => key.asJson.noSpaces.getBytes.pure[F])
  ...
  given serializerContractDeleteRequestedKey[F[_]: Async]
      : Serializer[F, ContractDeletedEventKey] =
    Serializer.lift(key => key.asJson.noSpaces.getBytes.pure[F])
